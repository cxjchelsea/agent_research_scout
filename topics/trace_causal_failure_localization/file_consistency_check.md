# File Consistency Check: Agentic Regression Scenarios for Tool-Using Agent Failures
...
|------|--------|-----------------|
| topic_brief.md | consistent | 已同步 Phase 3.5b 再次收窄题 |
...
| decision.md | ready | Narrow；不升 Promising |
...
|------|-------------|--------------|----------|----------|
| 课题名称 | Agentic Regression Scenarios for Tool-Using Agent Failures | Agentic Regression Scenarios for Tool-Using Agent Failures | Agentic Regression Scenarios for Tool-Using Agent Failures | 是 |
| 核心 claim | failure trace → replayable agentic scenario | agent-specific scenario artifact | Narrow | 是 |
| MVP benchmark | AgentRx / TraceElephant / TRAIL traces | 10–20 examples first | draft | 是 |
| 当前 Decision | — | 原题 No-Go；第一版收窄题不升 Promising；再次收窄题 Narrow | Narrow | 是 |
...
| 最大威胁是否覆盖核心 claim | partial / yes | 原始 localization claim 被覆盖；第一版 repair-test claim 被软件测试强压缩；再次收窄题未完全覆盖。 |
...
1. 不得以原始 trace localization claim 继续。
2. 不得做宽泛 software failure trace → regression test。
3. 若继续，只做 10–20 条 agentic regression scenario 样例，检查 tool mocks / policy / memory / handoff / oracle 是否能表达。
4. experiment_plan 保持 draft，不进入 Phase 4 execution。
...
审计结论：Phase 3.5b 对第一版收窄题给出谨慎负面结果，原因是传统软件测试已有强相关工作。当前只能再次收窄并维持 Narrow；experiment_plan 仍保持 draft，不进入 Phase 4 execution。
