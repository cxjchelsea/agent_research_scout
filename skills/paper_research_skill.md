...
```text
广搜发现 → 提出候选 → 选定一个方向 → 针对性深研 → 最大威胁定向核验 → 最小实验验证
```
...
3. 最大威胁 work 是否已经完成 **claim-level targeted verification**？
4. 能否用公开数据或可构造 benchmark 做 **MVP / pilot** 快速验证？
5. 我当前能力是否能在 **2–4 个月**内启动最小实验？
...
| `selection.md` | 用户最终选中的方向与 slug（Agent 不得替用户做最终选择） | `templates/selection_template.md` |

发现阶段 **不要求** `paper_cards/`。候选方向需在 table 和 `candidates.md` 中有足够论据；每个候选至少 3 篇 discovery table 条目，最大威胁 work 须 verified。
...
| 9 | `experiment_plan.md` | MVP 实验设计草案或 ready 版（受 Phase 3.5 gate 约束） | `templates/experiment_plan_template.md` |
...
- 最大威胁 1–3 篇 work 的 **claim coverage matrix**
- 是否只是换名
- 若收窄：完整收窄记录（原始题目 → 新题目 → 删除/保留/实验边界）
- **不得**嵌入 adversarial 或 consistency 全文（分别写入独立文件）

### claim coverage matrix 必须包含

对每篇最大威胁 work，至少填写：

| Claim component | Covered by threat work? | Evidence | Implication |
|---|---|---|---|
...
如果 Evidence 为空、只写“看起来没有覆盖”、或没有引用具体方法边界，不得视为 claim-level verification 完成。
...
- **Promising / Narrow**：可写 **MVP / pilot draft**（明确 minimum evidence），目标是快速验证创新点
- 如果最大威胁 work 尚未完成 Phase 3.5 targeted verification，`experiment_plan.md` 的状态只能是 **draft**，不得进入 Phase 4 execution
...
- 给出 3–7 个有论据的候选方向（`scout/candidates.md`），每个候选在 `discovery_paper_table.csv` 中至少有 3 篇来源支撑；
...
- 判断不是换名、不是已被完整覆盖；
- 对最大威胁 1–3 篇 work 完成 claim-level targeted verification。
...
| 3.5 | Targeted Verification | 对最大威胁 work 做 claim-level 核验 | 最危险 1–3 篇 paper card 达到 `threat_verified`，且有 claim coverage matrix | Proceed / Narrow / Hold / No-Go |
| 4 | Quick Proof | 用 MVP 快速证明该点值得做 | `experiment_plan.md` 为 ready + `outputs/` 有 minimum evidence | **Go** / Hold / No-Go |
...
3. 提出 **3–7 个候选方向**，写入 `candidates.md`；为每个候选分配 **candidate_id**（如 C1、C2），与 `discovery_paper_table.csv` 对齐；每个候选必须：
   - 有一句话定义；
   - 说明问题是否真实；
   - 在 `discovery_paper_table.csv` 中至少有 **3 篇** verified 或 uncertain 来源支撑；
   - 指出最大威胁 work（**必须 verified**；uncertain 只能作风险线索，不能作核心否定依据）；
...
2. 等用户确认后，按 `templates/selection_template.md` 写入 `scout/selection.md`；
...
8. decision.md
9. experiment_plan.md       （Promising / Narrow 时只能先写 draft；受 Phase 3.5 gate 约束）
```
...
- **Promising / Narrow**：可以写 MVP draft，但必须通过 Phase 3.5 才能进入 Phase 4；
- **Hold / No-Go**：停止或回到 `scout/` 重新发现。

### Phase 3.5 — Targeted Verification（最大威胁定向核验）

目标：对最大威胁 work 做 **claim-level verification**，确认当前创新点是否仍成立。

必须完成：

1. 最危险 1–3 篇 work 必须完成 targeted close reading。
2. 每篇最大威胁 work 的 paper card 必须达到 `threat_verified`。
3. `gap_analysis.md` 必须包含 claim coverage matrix。
4. 每篇最大威胁 work 必须回答：
   - 是否覆盖本课题核心 claim？
   - 是否覆盖本课题 proposed mechanism？
   - 是否覆盖本课题 benchmark / evaluation setting？
   - 如果扩展已有方法，是否自然得到本案方法？
5. 如果最大威胁 work 覆盖核心 claim，decision 必须降级为 **No-Go** 或重新 **Narrow**。
6. 未完成 targeted verification 前：
   - 不得把 Phase 3 标记为 complete；
   - 不得进入 Phase 4 execution；
   - `experiment_plan.md` 状态只能是 **draft**。
...
- `experiment_plan.md` 必须标明 **Experiment Plan Status: draft / ready**；只有 ready 才能执行 Phase 4；
...
- 最大威胁 work 未完成 `threat_verified` 和 claim coverage matrix 前，**不得**把 Phase 3 判定为 complete
- targeted verification 未完成前，`experiment_plan.md` 只能是 **draft**，不得进入 Phase 4 execution
...
- **candidate_id**（如 C1、C2，与 `candidates.md` 对齐）
- title, year, venue, category, type
- problem, benchmark, metrics, limitation
- **relevance_to_candidate**, relevance_score（说明与哪个候选方向相关；可与 candidate_id 并用）
...
- **stub**：只建了文件，信息不全；不得算作已精读。
- **metadata_verified**：标题、作者、年份、venue、链接已确认；只能说明论文存在。
- **close_read**：问题、方法、实验、limitation、关键引用已填；可计入“核心论文已精读”。
- **threat_verified**：已经完成 claim-level verification，明确判断是否覆盖本课题核心 claim、机制和 benchmark；仅最大威胁 work 需要达到此状态。

硬规则：

1. “有 paper card” 不等于 “已精读”。
2. Phase 3 统计核心精读时，只能计入 `card_status=close_read` 或 `threat_verified`。
3. 最大威胁 1–3 篇 work 必须达到 `threat_verified`，否则 Phase 3 不得判定 complete。
4. `verified_status=verified` 只表示论文 metadata / 来源核验，不表示 claim-level verification。
5. 如果 paper card 中仍有 “作者待确认 / 重要引用句待补 / 是否覆盖 claim 待确认” 等内容，则状态不得高于 `metadata_verified`。
...
```text
file consistency：文件之间是否矛盾
evidence readiness：证据是否足够支持当前 decision
```

仅通过 file consistency 不代表 Phase 3 complete；最大威胁 work 未完成 targeted verification 时，evidence readiness 必须标为 not ready。
...
11. 如果 core_read=yes 论文的 paper card 低于 `close_read`，不得计入 Phase 3 精读完成。
12. 如果最大威胁 work 的 paper card 低于 `threat_verified`，Phase 3 不得标记 complete，decision 不得标 Promising。
13. 如果缺少 claim coverage matrix，Phase 3 不得标记 complete。
14. 如果 experiment_plan 状态为 ready 但 targeted verification 未完成，必须降级为 draft。

### file_consistency_check.md 必须包含

1. 文件状态总表（consistent / outdated / risky）
2. 上述 14 条规则逐项 Pass/Fail
3. 跨文件关键字段对齐表（课题名称、核心 claim、MVP benchmark、Decision）
4. evidence readiness 检查表（paper card 状态、最大威胁核验、claim coverage matrix、experiment_plan 状态）
5. 发现的问题与待办
6. 审计结论
...
满足以下**全部**条件，才可标 **Promising** 或 **Narrow**（可写 MVP draft，但还不是 Go）：

1. 至少 8 篇核心论文 `metadata_verified`。
2. 至少 5 篇核心论文 `card_status=close_read` 或 `threat_verified`（通常 8/8）。
3. 最危险 1–3 篇 work 必须 `card_status=threat_verified`。
4. 最大威胁 work 必须有 claim coverage matrix，且 Evidence 不为空。
5. gap_analysis 明确指出已有工作覆盖了什么、没有覆盖什么。
6. adversarial_review 已完成，且 novelty / evaluation 风险有预备回应。
7. file_consistency_check 的 file consistency 与 evidence readiness 均通过。
8. 最危险的 3 篇 related work 已经被正面比较。
9. 课题已经收窄到一个可实验场景。
10. 有明确 benchmark 或可构造数据。
11. 有 baseline 与至少 3 个可量化指标。
...
如果最大威胁 work 尚未 `threat_verified`，decision 只能是 **Hold** 或 **Narrow-draft（写作时仍用 Narrow，但必须在 Reason 中标注 targeted verification 未完成）**，不得写 Promising，不得进入 Phase 4 execution。
...
- **Promising / Narrow**：Phase 3 结论；只有通过 Phase 3.5 targeted verification 后，才可进入 Phase 4 Quick Proof。
...
5. **先 targeted verification，再进入 Phase 4**（最大威胁 work 未 `threat_verified` 前，不得执行实验）。
6. **先 MVP，再声称 Go**（Go 只属于 Phase 4，且只表示值得继续投入）。
7. 不把 uncertain 论文当核心证据。
8. 不把 paper card 存在当作 close reading 完成。
9. 不把 metadata verified 当作 claim verified。
10. 不为了保留课题而降低判断标准。
11. 不把工程集成包装成研究贡献。
12. 每轮调研结束必须更新对应阶段文件；Phase 3 结束时更新 `file_consistency_check.md` 和 `decision.md`。
13. 所有结论以 repo 文件为准；对话摘要不得与文件矛盾。
14. **不做投稿审计**：40–60 篇扩库、论文级 Related Work 完整性不在本 skill 范围内。
